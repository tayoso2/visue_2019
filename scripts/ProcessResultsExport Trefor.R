...
MultipleMakeLong <-
  function(files,
           scenarios = NULL,
           measures = c("Quantity Resource","Age", "Intervention Costs", "Future PoF", "Financial Consequence Cost","Safety Consequence Cost", 
...
### Examples ###
asset.attributes <- read.csv("./Trefor/Model/NPI Assets.csv" , check.names = FALSE)
asset.attributes <- asset.attributes[, 1:20]

# Convert several files to long in one operation
long.result <-
  MultipleMakeLong(
    c("./Trefor/Model/NPI Assets.csv","./Trefor/Model/Maintain Risk Assets.csv"),
    scenarios = c("No Proactive Investment","Maintain Asset Risk"),
    attributes = c("Asset_Number")
  )

#semi.long.results <- MakeSemiLong(long.results)

write.csv(long.result, "./Output Data/Trefor long_results.csv", na = "")
write.csv(asset.attributes, "./Output Data/Trefor attributes.csv", na = "")
